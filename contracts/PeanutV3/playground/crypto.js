...
async function contractTests() {
    ...
    var recipientAddress = "0xd3222f01b4154528ceC2807D99385d0fA4473A31";
    // create recipient address hash
    /// 1. hash plain address - 2. hash of (prefix + hash)
    const recipientAddressHash1 = solidityHashAddress(recipientAddress);
    const recipientAddressHash1binary = ethers.utils.arrayify(recipientAddressHash1);
    const recipientAddressHash2 = solidityHashBytesEIP191(recipientAddressHash1binary);  // add prefix and hash (this is what we'll sign)
    console.log("** plain recipient address: ", recipientAddress);
    console.log("** recipient address hash 1 (no prefix): ", recipientAddressHash1);
    console.log("** recipient address hash 1 (no prefix) binary: ", recipientAddressHash1binary); // omfg finally jesus effing christ holy shit
    console.log("** recipient address hash 2 (with prefix) (FINAL HASH): ", recipientAddressHash2);

    // var _recipientAddressHash2 = ethers.utils.hashMessage(recipientAddress);
    // var _recipientAddressHash2bin = ethers.utils.hashMessage(recipientAddressHash1binary);
    // var _recipientAddressHash3 = _hashMessage(recipientAddress);
    // console.log("** plain recipient address: ", recipientAddress);
    // console.log("** recipient address hash 1 (no prefix): ", recipientAddressHash1);
    // console.log("** recipient address hash 1 (no prefix) binary: ", recipientAddressHash1binary); // omfg finally jesus effing christ holy shit
    // console.log("** recipient address hash 2 (with prefix): ", _recipientAddressHash2);
    // console.log("** recipient address hash 2 binary (with prefix): ", _recipientAddressHash2bin);
    // console.log("** recipient address hash 3 (with prefix): ", _recipientAddressHash3);
    // console.log("** random string prefix hashed: ", ethers.utils.hashMessage("hello world"));
    ...
    var signature2 = await signMessageWithPrivatekey(recipientAddress, keys.privateKey);
    var signature3 = await signMessageWithPrivatekey(recipientAddressHash1binary, keys.privateKey);
    var signature5 = await signAddress(recipientAddress, keys.privateKey);
    ...
    */
    ...
    console.log("signature 2: ", signature2);
    console.log("signature 3: ", signature3);
    console.log("signature 5: ", signature5);
